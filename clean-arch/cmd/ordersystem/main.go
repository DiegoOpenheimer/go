...
func main() {
	...
	webOrderHandler := NewWebOrderHandler(db, createOrderUseCase, listOrdersUseCase)
	server.Router.Post("/order", webOrderHandler.Create)
	server.Router.Get("/order", webOrderHandler.List)
